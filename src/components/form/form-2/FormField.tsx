...
type FormValues = { name: string; email: string; phone: string; education: string; address: string; message: string };
const emptyValues: FormValues = { name: "", email: "", phone: "", education: "", address: "", message: "" };
    ...
    <form onSubmit={submit} className="grid gap-3 rounded-sm border border-stone-200 bg-white p-5">
      ...
      {data.showEducation !== "false" && (
        <input
          ...
          onChange={(event) => setValues((current) => ({ ...current, education: event.target.value }))}
          placeholder={data.educationPlaceholder ?? defaultData.educationPlaceholder}
          className="rounded-sm border p-2"
        />
      )}
      {data.showAddress !== "false" && (
        <textarea
          ...
          placeholder={data.addressPlaceholder ?? defaultData.addressPlaceholder}
          className="min-h-20 rounded-sm border p-2"
        />
      )}
