...
export function HeroSection() {
  return (
    <section className="relative isolate overflow-hidden border-b border-[#dceafb] bg-[#f8fbff]">
      ...
      />
      <div aria-hidden className="absolute -left-44 top-8 h-80 w-80 rounded-sm bg-cyan-300/25 blur-3xl" />
      <motion.div
        animate={{ x: [0, 34, 0], y: [0, -24, 0], scale: [1, 1.1, 1] }}
        aria-hidden
        className="absolute -right-28 top-[-5rem] h-96 w-96 rounded-sm bg-blue-500/20 blur-3xl"
      ...
      <div className="relative mx-auto grid min-h-[760px] max-w-7xl items-center gap-14 px-5 py-16 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:gap-10 lg:px-10 lg:py-20 xl:px-0">
        <div className="relative z-10 max-w-2xl">
          ...
          >
            <a
              className="group inline-flex min-h-12 items-center justify-center gap-2 rounded-sm bg-[#087af5] px-5 text-sm font-bold text-white shadow-[0_14px_30px_rgba(8,122,245,0.27)] transition duration-300 hover:-translate-y-1 hover:bg-[#0065d7] hover:shadow-[0_20px_40px_rgba(8,122,245,0.34)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#087af5]"
            ...
            <a
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-sm border border-[#cfe2fb] bg-white/80 px-5 text-sm font-bold text-[#155caf] shadow-sm transition duration-300 hover:-translate-y-1 hover:border-[#087af5] hover:bg-white hover:text-[#086fe5] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#087af5]"
              href={WHATSAPP_AUDIT_URL}
              rel="noopener noreferrer"
              target="_blank"
            >
        ...
        >
          <div
            aria-hidden
            className="absolute -inset-5 rounded-sm bg-gradient-to-br from-blue-400/30 via-cyan-300/10 to-violet-400/25 blur-2xl"
          />
          <div className="relative overflow-hidden rounded-sm border border-white/80 bg-white p-3 shadow-[0_30px_80px_rgba(16,78,158,0.2)] sm:p-4">
            ...
            <div className="mt-3 grid grid-cols-[4.3rem_1fr] gap-3 sm:grid-cols-[5.6rem_1fr] sm:gap-4">
              <aside className="rounded-sm bg-[#0b1736] p-2.5 text-white sm:p-3">
                <div className="mb-6 flex size-7 items-center justify-center rounded-sm bg-white/10">
                  <LayoutDashboard className="size-3.5" />
                </div>
                <div className="space-y-3">
                  {[LayoutDashboard, UsersRound, BarChart3, Bot].map((DashboardIcon, index) => (
                    <div
                      className={`flex h-7 items-center gap-2 rounded-sm px-1.5 ${index === 0 ? "bg-white/12" : "text-slate-400"}`}
                      key={index}
                    >
                ...
                <div className="mt-12 hidden rounded-sm border border-white/10 bg-white/5 p-2 text-[0.55rem] leading-3 text-slate-400 sm:block">
                  Your team knows
                  <br />
                  what comes next.
                </div>
              ...
              <div className="min-w-0 py-1 pr-1">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-sm bg-[#e8f3ff] px-2 py-1 text-[0.58rem] font-bold text-[#087af5]">
                    September
                  </span>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2.5 sm:grid-cols-3">
                  {[
                    ...
                    <div className="rounded-sm border border-slate-100 bg-[#fbfdff] p-2.5 shadow-sm" key={label}>
                ...
                <div className="mt-3 grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
                  <div className="rounded-sm border border-slate-100 p-3">
                    ...
                    <div className="mt-4 flex h-16 items-end gap-1.5">
                      {[34, 58, 43, 78, 51, 88, 64, 94, 72].map((height, index) => (
                        <motion.span
                          ...
                          className={`w-full rounded-sm ${index === 7 ? "bg-[#087af5]" : "bg-blue-100"}`}
                          initial={{ height: 0 }}
                          key={index}
                  ...
                  <div className="rounded-sm bg-[#eff9ff] p-3">
                    <p className="text-[0.62rem] font-bold text-[#1764be]">Next action</p>
                    <div className="mt-3 flex items-center gap-2">
                      <div className="flex size-7 items-center justify-center rounded-sm bg-white text-[0.58rem] font-extrabold text-[#087af5]">
                        SR
                      </div>
                      <div>
                    ...
                    <div className="mt-3 h-1.5 overflow-hidden rounded-sm bg-blue-100">
                      <motion.div
                        animate={{ width: ["24%", "72%", "52%"] }}
                        className="h-full rounded-sm bg-[#08b7d7]"
                        transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                      />
                    </div>
                  </div>
                </div>
                <div className="mt-3 flex items-center justify-between rounded-sm border border-slate-100 px-3 py-2">
                  <div className="flex items-center gap-2">
          ...
          <motion.div
            animate={{ y: [0, -7, 0] }}
            className="absolute -bottom-5 -left-4 rounded-sm border border-white bg-white px-3.5 py-3 shadow-[0_14px_35px_rgba(10,72,150,0.16)] sm:-left-9"
            transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
          >
            <div className="flex items-center gap-2">
              <span className="flex size-8 items-center justify-center rounded-sm bg-[#e8f8fb] text-[#08a6c2]">
