...
import { defaultDataSection47, defaultLayout, type Section47Data, type Section47Payload } from "./data";
...
export interface Section47FormProps {
  data?: Section47Data | Section47Payload;
  onChange?: (values: Section47Payload) => void;
}
...
const MutationSection47 = ({ data, onChange }: Section47FormProps) => {
  const onChangeRef = useRef(onChange);
  const initialPayload = {
    ...defaultDataSection47,
    ...defaultLayout,
    ...(data || {}),
    paragraphs: [...(data?.paragraphs || defaultDataSection47.paragraphs)],
  };
  const [formData, setFormData] = useState<Section47Data>(initialPayload);
  ...
  const updateField = <K extends keyof Section47Data>(field: K, value: Section47Data[K]) => {
      ...
      <div className="mx-auto w-full max-w-7xl overflow-hidden rounded-sm border-x border-[#eadfca] bg-white shadow-sm">
        <header className="flex items-center gap-3 border-b border-[#eadfca] bg-white p-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Manage the left image, right-side content, and section colors.</p>
          </div>
        </header>
        ...
        <div className="grid gap-8 p-6 md:p-8">
          ...
          <div className="space-y-7">
            <section className="space-y-4">
              <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-[0.16em] text-slate-500">
                <ImageIcon className="h-4 w-4" /> Left image
              </h3>
              <div className="space-y-5 rounded-sm border border-[#eadfca] bg-[#fffaf0] p-5">
            ...
            <section className="space-y-4">
              <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-[0.16em] text-slate-500">
                <Type className="h-4 w-4 text-cyan-600" /> Spacing
              </h3>
              <div className="space-y-5 rounded-sm border border-cyan-100 bg-cyan-50/60 p-5">
                {(
                  [
