...
import { defaultDataSection23, defaultLayout, IFeatureCard, Section23Data, Section23Payload } from "./data";
...
export interface Section23FormProps {
  data?: Section23Data | Section23Payload;
  onChange?: (values: Section23Data | Section23Payload) => void;
  onSubmit?: (values: Section23Data | Section23Payload) => void;
}
...
const MutationSection23 = ({ data, onChange }: Section23FormProps) => {
  const onChangeRef = useRef(onChange);
  const initialPayload: Section23Payload =
    data && "paddingX" in data ? data : { ...defaultDataSection23, ...defaultLayout, ...(data || {}) };
  const [formData, setFormData] = useState<Section23Data>(() => ({
    ...initialPayload,
    cards: initialPayload.cards ?? defaultDataSection23.cards,
  }));
  ...
  const updateField = (field: keyof Section23Data, value: any) => {
      ...
      <div className="w-full overflow-hidden rounded-sm border border-[#e8d8bd] bg-[#fffdf8] shadow-[0_14px_35px_rgba(105,66,38,0.08)]">
        <div className="flex items-center gap-3 border-b border-[#eadcc7] bg-gradient-to-r from-[#fff2d9] via-[#fffaf0] to-[#f4efff] p-6">
          ...
          <div>
            ...
            <p className="text-sm text-[#765f50]">Manage your feature cards grid.</p>
          </div>
        </div>
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    className="min-h-[80px] resize-none border-[#e8d8bd] bg-[#fffdf8] text-sm text-[#49352b] focus:border-[#9a5b9c]"
                  />
                </div>
                ...
                <div className="space-y-3">
                  <div className="space-y-2">
                    ...
                    >
                      <span
                        className={`grid h-11 w-11 shrink-0 place-items-center rounded-sm bg-gradient-to-br ${card.gradient} text-white shadow-sm`}
                      >
                        {(() => {
                          const CardIcon = iconMap[card.iconName] ?? iconMap.Zap;
                          return <CardIcon size={20} strokeWidth={2} />;
                        })()}
                      </span>
                    ...
                    <IconPicker label="Card icon" onChange={(icon) => updateCard(idx, "iconName", icon)} value={card.iconName} />
                  </div>

                  <div className="space-y-2">
                    <Label className="text-xs text-[#765f50]">Theme</Label>
                    <select
                      value={card.gradient}
                ...
                <div className={`h-2 w-full rounded-full bg-gradient-to-r ${card.gradient} opacity-50`} />
              </div>
            ))}
          </div>
        </div>
            ...
            <>
              ...
              <div className="space-y-4 p-5 sm:p-6">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="rounded-full bg-[#f2e8ff] px-2.5 py-1 font-mono text-[#70458f]">
                      ...
                      {filteredIconOptions.map((iconName) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "group/icon flex aspect-square flex-col items-center justify-center rounded-sm border transition-all duration-200",
                              isSelected
