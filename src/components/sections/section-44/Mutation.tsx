...
export interface Section44FormProps {
  data?: Section44Data | Section44Payload;
  onChange?: (values: Section44Data | Section44Payload) => void;
  onSubmit?: (values: Section44Data | Section44Payload) => void;
}

const cloneData = (data: Section44Data): Section44Data => ({
  ...data,
  requirements: data.requirements.map((requirement) => ({ ...requirement })),
});

const normalizeData = (data?: Section44Data | Section44Payload): Section44Data => {
  if (!data) return cloneData(defaultDataSection44);
  return {
    ...defaultDataSection44,
    ...data,
    requirements: Array.isArray(data.requirements)
      ? data.requirements.map((requirement) => ({ ...requirement }))
      : defaultDataSection44.requirements.map((requirement) => ({ ...requirement })),
  };
};
...
const colorFields: Array<{
  ...
  { field: "headingColor", label: "Heading and requirement titles" },
  { field: "textColor", label: "Body text" },
  { field: "accentColor", label: "Check marks" },
];

const MutationSection44 = ({ data, onChange }: Section44FormProps) => {
  const onChangeRef = useRef(onChange);
  const initialPayload: Section44Payload = { ...defaultDataSection44, ...defaultLayout, ...(data || {}) };
  const [formData, setFormData] = useState<Section44Data>(() => normalizeData(data));
  ...
  const updateField = <K extends keyof Section44Data>(field: K, value: Section44Data[K]) => {
    setFormData((current) => ({ ...current, [field]: value }));
  };

  const updateRequirement = (id: string, field: keyof Omit<Section44Requirement, "id">, value: string) => {
    setFormData((current) => ({
      ...current,
  ...
  const addRequirement = () => {
    updateField("requirements", [
      ...
      {
        id: `requirement-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
  ...
  const removeRequirement = (id: string) => {
    updateField(
      "requirements",
      formData.requirements.filter((requirement) => requirement.id !== id),
    );
  };
    ...
    <div className="custom-parent-border min-h-screen bg-white text-slate-900 md:p-8">
      <div className="mx-auto w-full max-w-7xl overflow-hidden rounded-sm border-x border-[#eadfca] bg-white shadow-sm">
        <header className="flex items-center gap-3 border-b border-[#eadfca] bg-white p-6">
          ...
          <div>
            <h2 className="text-xl font-bold">Edit United Kingdom Visa Requirements Section</h2>
            <p className="text-sm text-slate-500">
              Manage the visa introduction, requirements, campus image, and colors.
            </p>
          </div>
        </header>
          ...
          <div className="space-y-7">
            ...
            <section className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-[0.16em] text-slate-500">
                  <FileCheck2 className="h-4 w-4" /> Requirements
                </h3>
                <Button
                  type="button"
                  variant="outline"
                ...
                {formData.requirements.map((requirement, index) => (
                  ...
                  >
                    ...
                    <div className="space-y-2 pr-8">
                      <Label>Requirement {index + 1} title</Label>
                      <Input
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        onChange={(event) => updateRequirement(requirement.id, "description", event.target.value)}
                        className={`${fieldClassName} min-h-36 resize-y`}
                        rows={6}
                      />
                    </div>
