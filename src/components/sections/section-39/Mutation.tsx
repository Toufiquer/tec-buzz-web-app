...
import { defaultDataSection39, defaultLayout, type Section39Data, type Section39Payload } from "./data";
...
export interface Section39FormProps {
  data?: Section39Data | Section39Payload;
  onChange?: (values: Section39Data | Section39Payload) => void;
  onSubmit?: (values: Section39Data | Section39Payload) => void;
}

const imageFields: Array<{ field: "primaryImage" | "topImage" | "bottomImage"; label: string }> = [
  { field: "primaryImage", label: "Primary image" },
...
const MutationSection39 = ({ data, onChange }: Section39FormProps) => {
  const onChangeRef = useRef(onChange);
  const initialPayload: Section39Payload = { ...defaultDataSection39, ...defaultLayout, ...(data || {}) };
  const [formData, setFormData] = useState<Section39Data>(() => ({
    ...initialPayload,
    paragraphs: [...(initialPayload.paragraphs || defaultDataSection39.paragraphs)],
  }));
  ...
  const updateField = <K extends keyof Section39Data>(field: K, value: Section39Data[K]) => {
    ...
    <div className="custom-parent-border min-h-screen bg-white text-slate-900 md:p-8">
      <div className="mx-auto w-full max-w-7xl overflow-hidden rounded-sm border-x border-[#eadfca] bg-white shadow-sm">
        <header className="flex items-center gap-3 border-b border-[#eadfca] bg-white p-6">
          ...
          <div>
            <h2 className="text-xl font-bold">Edit Visa Processing Section</h2>
