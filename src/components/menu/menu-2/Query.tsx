...
export default function MenuTwoQuery({ data, pending }: { data: MenuData; pending: boolean }) {
  ...
  const visibleLinks = data.links.filter((item) => item.visible);
  const activeItem = visibleLinks.find((item) => item.id === openItemId);
  const activeChildren = activeItem?.children?.filter((item) => item.visible) ?? [];
  ...
  return (
    <header
      className="relative z-50 border-b border-[#dce3ed]"
    ...
    >
      <div className="relative mx-auto flex min-h-[56px] max-w-7xl items-center justify-center  px-4 md:px-6 ">
        ...
        <nav aria-label="Primary navigation" className="hidden items-center gap-9 md:flex">
          {visibleLinks.map((item) => {
            const hasChildren = Boolean(item.children?.some((child) => child.visible));
      ...
      {activeItem && activeChildren.length ? (
        <div className="animate-in fade-in-0 slide-in-from-top-1 absolute left-0 top-full min-h-[400px] w-full border-b border-[#dce3ed] bg-white px-4 py-3 duration-200 ease-out shadow-[0_14px_24px_rgba(15,23,42,0.06)]">
          <div className="mx-auto grid max-w-[1160px] grid-cols-1 gap-x-2 gap-y-1 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div
        className={`absolute inset-x-0 top-full grid overflow-hidden border-b border-[#dce3ed] bg-[inherit] px-4 transition-[grid-template-rows,opacity,margin] duration-300 ease-out md:hidden ${mobileOpen ? "mt-0 grid-rows-[1fr] opacity-100" : "mt-0 grid-rows-[0fr] opacity-0"}`}
      >
        <nav className="mx-auto flex min-h-0 w-full max-w-7xl flex-col gap-1 py-3">
          {mobileLinks
