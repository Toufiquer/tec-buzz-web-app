...
import {
  ...
  type AboutTheCountryProps,
  type IAboutTheCountryData,
} from "./data";

const parseData = (data?: IAboutTheCountryData | AboutTheCountryPayload | string): AboutTheCountryPayload => {
  try {
    const incoming = typeof data === "string" ? (JSON.parse(data) as Partial<AboutTheCountryPayload>) : data;
    return {
      ...defaultDataAboutTheCountry,
      ...defaultLayout,
      ...incoming,
      ...
      pageName: "Home",
      sections: Array.isArray(incoming?.sections) ? incoming.sections : defaultDataAboutTheCountry.sections,
    };
  } catch {
    return { ...defaultDataAboutTheCountry, ...defaultLayout };
  }
};

const QueryAboutTheCountry = ({ data }: AboutTheCountryProps) => {
      ...
      <section className="custom-parent-border">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.15fr_0.85fr] lg:px-8 lg:py-20">
          ...
          <aside className="grid content-start gap-3 rounded-sm border border-[#eadfca] bg-amber-50/50 p-4">
            ...
            <p className="text-xl font-semibold text-stone-900">A clear path from first visit to next action.</p>
            <p className="text-sm leading-6 text-stone-600">
              Designed to make your service, proof, and next step easy to understand.
            </p>
          </aside>
        </div>
      </section>
...
export default QueryAboutTheCountry;
