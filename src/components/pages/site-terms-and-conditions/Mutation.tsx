...
export interface SiteTermsAndConditionsFormProps {
  data?: ISiteTermsAndConditionsData | SiteTermsAndConditionsPayload;
  onChange?: (values: SiteTermsAndConditionsPayload) => void;
}
const normalizeData = (
  data?: ISiteTermsAndConditionsData | SiteTermsAndConditionsPayload,
  ...
  sections: Array.isArray(data?.sections) ? data.sections : defaultDataSiteTermsAndConditions.sections,
});
const MutationSiteTermsAndConditions = ({ data, onChange }: SiteTermsAndConditionsFormProps) => {
  const onChangeRef = useRef(onChange);
  const initialPayload = normalizeData(data);
  const [formData, setFormData] = useState<ISiteTermsAndConditionsData>(initialPayload);
  ...
  const removeSection = () => {
    ...
    setDeleteIndex(null);
    setToast("Terms section removed.");
  };
  return (
    <div className="custom-parent-border bg-white text-slate-800">
      ...
      <div className="mx-auto max-w-7xl border-x border-[#eadfca]">
        <header className="flex flex-col gap-3 border-b border-[#eadfca] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <h2 className="truncate text-lg font-semibold text-stone-900">Edit Terms & Conditions</h2>
            <p className="truncate text-sm text-stone-600">
              site-terms-and-conditions-uid · Terms, responsibilities, and contact details
            </p>
          </div>
        </header>
        <ScrollArea className="max-h-[calc(100vh-12rem)] p-4">
          <div className="grid gap-4 pb-4 lg:grid-cols-2">
            <section className="grid gap-3 rounded-sm border border-slate-200 bg-slate-50 p-4 lg:col-span-2">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="grid gap-1.5">
                  <Label htmlFor="site-terms-and-conditions-name">Page name</Label>
                  <Input
                    id="site-terms-and-conditions-name"
                    value={formData.pageName}
                    onChange={(event) => setFormData((current) => ({ ...current, pageName: event.target.value }))}
                  />
                </div>
                <div className="grid gap-1.5">
                  <Label htmlFor="site-terms-and-conditions-eyebrow">Eyebrow</Label>
                  <Input
                    id="site-terms-and-conditions-eyebrow"
              ...
              <div className="grid gap-1.5">
                <Label htmlFor="site-terms-and-conditions-title">Title</Label>
                <Input
                  id="site-terms-and-conditions-title"
                  value={formData.title}
                  onChange={(event) => setFormData((current) => ({ ...current, title: event.target.value }))}
                />
              </div>
              <div className="grid gap-1.5">
                <Label htmlFor="site-terms-and-conditions-subtitle">Subtitle</Label>
                <Textarea
                  className="min-h-36"
                  id="site-terms-and-conditions-subtitle"
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="grid gap-1.5">
                  <Label htmlFor="site-terms-and-conditions-primary">Primary action</Label>
                  <Input
                    id="site-terms-and-conditions-primary"
                    value={formData.primaryAction}
                    onChange={(event) => setFormData((current) => ({ ...current, primaryAction: event.target.value }))}
                  />
                </div>
                <div className="grid gap-1.5">
                  <Label htmlFor="site-terms-and-conditions-secondary">Secondary action</Label>
                  <Input
                    id="site-terms-and-conditions-secondary"
            ...
            {formData.sections.map((section, index) => (
              <section
                className="grid gap-3 rounded-sm border border-slate-200 bg-white p-4 shadow-sm"
              ...
              >
                ...
                <div className="grid gap-1.5">
                  <Label htmlFor={`site-terms-and-conditions-section-eyebrow-${index}`}>Eyebrow</Label>
                  <Input
                    id={`site-terms-and-conditions-section-eyebrow-${index}`}
                    value={section.eyebrow}
                    onChange={(event) => updateSection(index, "eyebrow", event.target.value)}
                  />
                </div>
                <div className="grid gap-1.5">
                  <Label htmlFor={`site-terms-and-conditions-section-title-${index}`}>Title</Label>
                  <Input
                    id={`site-terms-and-conditions-section-title-${index}`}
                    value={section.title}
                    onChange={(event) => updateSection(index, "title", event.target.value)}
                  />
                </div>
                <div className="grid gap-1.5">
                  <Label htmlFor={`site-terms-and-conditions-section-description-${index}`}>Description</Label>
                  <Textarea
                    className="min-h-36"
                    id={`site-terms-and-conditions-section-description-${index}`}
                    value={section.description}
                    onChange={(event) => updateSection(index, "description", event.target.value)}
                  />
                </div>
                <div className="grid gap-1.5">
                  <Label htmlFor={`site-terms-and-conditions-section-items-${index}`}>Points (one per line)</Label>
                  <Textarea
                    className="min-h-36"
                    id={`site-terms-and-conditions-section-items-${index}`}
      ...
      <AlertDialog
        busy={false}
        description="This removes the terms section from the current editor."
        onCancel={() => setDeleteIndex(null)}
        onConfirm={removeSection}
        open={deleteIndex !== null}
        title="Delete this terms section?"
      />
    </div>
  );
};
export default MutationSiteTermsAndConditions;
