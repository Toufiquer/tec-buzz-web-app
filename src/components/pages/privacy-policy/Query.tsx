...
import { defaultDataPage4, type ISection4Data, type Section4Props } from "./data";
...
const sectionIcons = ["Database", "FileText", "Lock", "ShieldCheck"];
...
const QueryPage4 = ({ data }: Section4Props) => {
  ...
  useEffect(() => {
    const updateActiveSection = () => {
      const firstVisibleSection = sections.find((_, index) => {
        const element = document.getElementById(`privacy-${index + 1}`);
  ...
  const selectSection = (index: number) => {
    ...
    document.getElementById(`privacy-${index + 1}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
      ...
      <section className="bg-[#fffaf0] px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[1fr_.7fr] lg:text-left">
          <div className="text-center lg:text-left">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-sm bg-amber-100 text-amber-800 shadow-sm lg:mx-0">
              {iconMap.Lock}
            </div>
            {pageData.showEyebrow && pageData.eyebrow && (
              <p className="mt-5 text-xs font-bold uppercase tracking-[.2em] text-amber-800">{pageData.eyebrow}</p>
            )}
            <h1 className="mt-5 text-4xl font-semibold tracking-tight text-slate-950 sm:text-5xl">{pageData.title}</h1>
      ...
      <section className="px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl rounded-sm border border-amber-200 bg-[#fff8e8] p-6 text-center sm:p-8">
          <p className="text-sm leading-7 text-slate-600">
            If you have a privacy question or would like to make a request about your information, please contact Speed
            Box through our official support channel.
          </p>
        </div>
      </section>
    </main>
