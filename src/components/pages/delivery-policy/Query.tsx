...
import {
  ...
  type DeliveryPolicyProps,
  type IDeliveryPolicyData,
} from "./data";

const parseData = (data?: IDeliveryPolicyData | DeliveryPolicyPayload | string): DeliveryPolicyPayload => {
  try {
    const incoming = typeof data === "string" ? (JSON.parse(data) as Partial<DeliveryPolicyPayload>) : data;
    return {
      ...defaultDataDeliveryPolicy,
      ...defaultLayout,
      ...incoming,
      ...
      sections: Array.isArray(incoming?.sections) ? incoming.sections : defaultDataDeliveryPolicy.sections,
    };
  } catch {
    return { ...defaultDataDeliveryPolicy, ...defaultLayout };
  }
};
const QueryDeliveryPolicy = ({ data }: DeliveryPolicyProps) => {
    ...
    >
      <section className="custom-parent-border">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.15fr_0.85fr] lg:px-8 lg:py-20">
          <div className="min-w-0">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              ...
              <Button
                ...
                variant="outline"
              >
                {iconMap.Package} Track delivery
              </Button>
            </div>
          </div>
          <aside className="grid content-start gap-3 rounded-sm border border-[#eadfca] bg-amber-50/50 p-4">
            ...
            <p className="text-xl font-semibold text-stone-900">Delivery information, timing, tracking, and support.</p>
      ...
      <section className="border-x border-[#eadfca] border-t">
        <div className="mx-auto max-w-7xl p-4">
          <p className="text-xs font-semibold tracking-[0.14em] text-amber-800 uppercase">Need help?</p>
          <h2 className="mt-1 text-xl font-semibold text-stone-900">{pageData.helpTitle}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">{pageData.helpDescription}</p>
          <a
...
export default QueryDeliveryPolicy;
