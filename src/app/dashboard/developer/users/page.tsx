...
import {
  type UserItem as User,
  useDeleteUserMutation,
  useDeleteUsersMutation,
  useGetUsersQuery,
  useUpdateUserMutation,
} from "@/redux/features/dashboard/users/usersSlice";
...
export default function DeveloperUserPage() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetUsersQuery({ page, limit: pageSize, ...(search.trim() ? { q: search.trim() } : {}) }, { skip: !session });
  const [updateUser, { isLoading: saving }] = useUpdateUserMutation();
  const [deleteUser, { isLoading: deletingOne }] = useDeleteUserMutation();
  const [deleteUsers, { isLoading: deletingMany }] = useDeleteUsersMutation();
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(
      () => setToast({ text: getErrorMessage(usersError, "Could not load users."), type: "error" }),
      0,
    );
    return () => window.clearTimeout(timer);
  }, [usersError]);
  ...
  async function loadUsers() {
    ...
    try {
      ...
      setSelectedUsers({});
      setPage(1);
      setToast({ text: "Users refreshed successfully.", type: "success" });
    } catch (error) {
      showError(getErrorMessage(error, "Could not load users."));
    }
  }
  const total = usersData?.total ?? 0;
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const allVisibleSelected = users.length > 0 && users.every((item) => selected.includes(item.id));
  async function removeUsers(ids: string[]) {
    if (!ids.length || !(await confirmDelete(`Delete ${ids.length} user${ids.length === 1 ? "" : "s"}?`))) return;
    try {
      if (ids.length === 1) await deleteUser(ids[0]).unwrap();
      else await deleteUsers(ids).unwrap();
      setMessage("");
      ...
      setToast({ text: `${ids.length} user${ids.length === 1 ? "" : "s"} deleted successfully.`, type: "success" });
    } catch (error) {
      showError(getErrorMessage(error, "Could not delete users."));
    }
  }
  async function saveUser(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!editing) return;
    try {
      await updateUser(editing).unwrap();
      setMessage("");
      setEditing(null);
      setSelectedUsers((current) => (current[editing.id] ? { ...current, [editing.id]: editing } : current));
      setToast({ text: "User updated successfully.", type: "success" });
    } catch (error) {
      showError(getErrorMessage(error, "Could not update user."));
    }
  }
  function toggleSelection(user: User) {
    setSelected((current) =>
      current.includes(user.id) ? current.filter((item) => item !== user.id) : [...current, user.id],
    );
    setSelectedUsers((current) => {
      if (current[user.id]) {
        const next = { ...current };
  ...
  function toggleVisibleSelection() {
    ...
    setSelectedUsers((current) => {
      ...
      else users.forEach((user) => (next[user.id] = user));
      return next;
    });
  }
  async function exportUsers() {
    const exportItems = Object.values(selectedUsers);
    ...
    try {
      setExporting(true);
      const spreadsheetName = `users-${timestamp}.xlsx`;
      const archive = createZip([{ name: spreadsheetName, content: createXlsx(exportItems) }]);
      const downloadUrl = URL.createObjectURL(new Blob([archive], { type: "application/zip" }));
      const link = document.createElement("a");
      link.href = downloadUrl;
      link.download = `users-export-${timestamp}.zip`;
      ...
      setMessage("");
      setToast({
        text: `${exportItems.length} user${exportItems.length === 1 ? "" : "s"} exported successfully.`,
        type: "success",
      });
    } catch (error) {
      showError(getErrorMessage(error, "Could not export users."));
    } finally {
      setExporting(false);
    }
  }
  if (sessionPending)
    return (
      <main className="flex flex-1 items-center justify-center bg-[#fffaf0]">
        <UserPageLoader label="Preparing your workspace" />
      </main>
    );
  if (!session)
    return (
  ...
  return (
    <main className="flex-1 bg-[#fffaf0] px-4 py-8 sm:px-6 lg:px-10">
      {pageBusy && <UserPageLoader label={loadingLabel} overlay />}
      {toast && (
        <div
        ...
        <div className="relative overflow-hidden rounded-sm border border-[#eadfca] bg-white p-5 shadow-[0_20px_60px_-35px_rgba(120,53,15,.35)] sm:p-7">
          ...
          {selected.length > 0 && (
            <div className="relative mt-5 flex items-center justify-between gap-3 rounded-sm bg-amber-100 px-3 py-2">
              ...
              <button
                ...
                disabled={deleting}
                onClick={() => void removeUsers(selected)}
                type="button"
              >
          ...
          {(message || usersError) && (
            <p className="relative mt-5 rounded-sm bg-red-50 px-3 py-2 text-sm text-red-700" role="status">
              {message || getErrorMessage(usersError, "Could not load users.")}
            </p>
          )}
              ...
              <table className="w-full table-fixed text-left text-sm">
                <thead className="bg-[#f8f0df] text-xs uppercase tracking-wider text-stone-500">
                  <tr>
                    ...
                    <th className="w-[22%] px-4 py-3">Mobile</th>
                ...
                <tbody>
                  {users.map((item) => (
                    <tr className="border-t border-stone-100 transition hover:bg-amber-50/40" key={item.id}>
                      ...
                      <td className="truncate px-4 py-3 font-medium text-stone-800" title={item.name}>
                        {item.name}
                      </td>
                      <td className="truncate px-4 py-3 text-stone-600" title={item.email}>
                        {item.email}
                      </td>
                      <td className="truncate px-4 py-3 text-stone-500" title={item.mobileNumber || "—"}>
                        {item.mobileNumber || "—"}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex justify-end gap-2">
                          <button
                            ...
                            className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-sm border border-stone-200 text-stone-600 hover:bg-amber-100"
                          ...
                          <button
                            ...
                            className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-sm border border-red-100 text-red-700 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                            disabled={deleting}
                            onClick={() => void removeUsers([item.id])}
                            type="button"
                          >
                            <Trash2 className="h-4 w-4" />
                          </button>
                ...
                <article className="min-w-0 rounded-sm border border-stone-200 p-3" key={item.id}>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex shrink-0 gap-1">
                      <button
                        aria-label="View user"
                        className="inline-flex h-8 w-8 cursor-pointer items-center justify-center text-stone-600"
                        onClick={() => setViewing(item)}
                        type="button"
                      >
                        <Eye className="h-4 w-4" />
                      ...
                      <button
                        aria-label="Delete user"
                        className="inline-flex h-8 w-8 cursor-pointer items-center justify-center text-red-700 disabled:cursor-not-allowed disabled:opacity-50"
                        disabled={deleting}
                        onClick={() => void removeUsers([item.id])}
                        type="button"
                      >
                  ...
                  <p className="mt-3 truncate text-sm text-stone-600" title={item.email}>
                    {item.email}
                  </p>
                  <p className="mt-1 truncate text-xs text-stone-400" title={item.mobileNumber || "—"}>
                    {item.mobileNumber || "—"}
                  </p>
                </article>
              ))}
            ...
            <div className="relative mt-4 flex flex-wrap items-center justify-between gap-3 text-sm text-stone-600">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-sm border border-stone-200 disabled:cursor-not-allowed disabled:opacity-40"
                  disabled={page === 1}
      ...
      {viewing && <UserModal user={viewing} onClose={() => setViewing(null)} />}
      {editing && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-stone-950/20 p-4 backdrop-blur-sm">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button
                aria-label="Close"
                className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-sm text-stone-500 hover:bg-amber-100"
                disabled={saving}
            ...
            <label className="mt-4 block text-sm font-medium text-stone-700">
              ...
              <input
                className="mt-1.5 w-full rounded-sm border border-stone-200 bg-white px-3 py-2.5 outline-none focus:border-amber-600"
                onChange={(event) => setEditing({ ...editing, mobileNumber: event.target.value })}
            ...
            <label className="mt-4 flex items-center gap-2 text-sm font-medium text-stone-700">
              <input
                ...
                className="cursor-pointer"
                onChange={(event) => setEditing({ ...editing, emailVerified: event.target.checked })}
                type="checkbox"
              />
              Email verified
            </label>
...
function createXlsx(users: User[]) {
  const headers = ["ID", "Name", "Email", "Mobile", "Email verified", "Image", "Created at", "Updated at"];
    ...
    .map(
      (row, rowIndex) =>
        `<row r="${rowIndex + 1}">${row
          .map(
            (value, columnIndex) =>
              `<c r="${columnName(columnIndex)}${rowIndex + 1}" t="inlineStr"><is><t xml:space="preserve">${escapeXml(safeCell(value))}</t></is></c>`,
          )
          .join("")}</row>`,
    )
    .join("");
  ...
  return createZip([
    ...
    },
    {
      ...
      content: encode(
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Users" sheetId="1" r:id="rId1"/></sheets></workbook>',
      ),
    },
    {
...
function UserPageLoader({ label, overlay = false }: { label: string; overlay?: boolean }) {
  return (
    <div
      aria-live="polite"
